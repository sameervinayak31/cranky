...
- Sheet tabs: `Log`, `Beans`, `Options`.
- Beans columns: `name | roaster | notes | grind | grams | opened | status` — id = sheet row number (beans are archived, never deleted, so ids stay stable).
- Options columns: `category | value` where category ∈ {drink, barista, person}.
- Client caches `getData` in localStorage (stale-while-revalidate → instant reopens). Maintenance edits are optimistic (apply locally, sync in background, self-heal on failure). Grams floored at 0 on client and server. Grams/grind edits commit on a debounce (don't rely on the flaky change/blur event). Chart.js is lazy-loaded only on the Stats tab.
