...
- Maintenance gained a "Drink log" popup listing recent orders with a delete button per entry. Deleting a log entry also refunds its grams to the matching bean, matched via `beanId` (see below). New `deleteLog` Code.gs action; log entries now carry a stable `id` (sheet row number), same pattern as beans. The popup opens instantly from the cached log rather than blocking on a full refresh (which was the actual source of it feeling slow — `getData()` ships the whole log every time, and that gets more expensive as history grows, not the popup's own 40-entry render). A background refresh reconciles afterward; an entry logged seconds ago and not yet confirmed with a real row-number id shows "syncing…" instead of a delete button until that reconcile lands. Full pagination was considered and explicitly rejected — the use case (fixing a recent mistake) never needs deep history. If the felt slowness comes back as the log grows over years, the next lever is a dedicated lightweight backend action that reads only the last ~50 Log rows directly (cheap regardless of total sheet size) instead of routing through the full `getData()` — not built yet, deliberately deferred.
- **Bug fix:** the Stats "Drinks by country" pie chart (and the log-delete grams refund) matched a log entry to its bean by *name*, but `Log.bean` is just a snapshot of the bean's name at order time — renaming a bean orphaned every historical log entry for it, which is what caused renamed Ethiopia beans to show as "Unspecified." Fixed at the root: `Log` gained an 8th column, `beanId` (the bean's stable row number, already known at log time as `p.beanId` — no extra lookup needed), and both the pie chart and the delete-refund now prefer `beanId` over name, falling back to name-matching only for log rows written before this fix shipped. Renaming a bean no longer requires touching the Log at all — no cascade-rewrite needed, which was the first approach considered and rejected as more expensive and less stable than just storing the id at write time. **Known limitation:** log entries created *before* this fix has no `beanId` and can't be retroactively linked (the log only ever stored the name, and once a bean's renamed the old name isn't recoverable from anywhere) — those rows stay "Unspecified" permanently unless manually fixed (e.g. a one-time Find & Replace on the Log sheet's bean column, swapping the old name for the current one, if the old name is still known).
...
- Duplicate bean names are expected and handled deliberately (the same coffee gets bought again regularly) — audited every bean-name-matching site after the beanId fix to confirm this. Ordering/log-delete-refund/country-pie-chart all key off the bean's stable id already, so two active bags sharing a name are never confused for data purposes. The one gap found: Order-page bean tiles only showed the name + grams, so two same-named active bags were visually indistinguishable at a glance. Fixed — tiles for a duplicate bag (reusing `beanGroupKey`, same name+roaster+country grouping as the Beans-page carousel) now tag the bean's roast date if it has one set; if that bag has no roast date, no tag shows for it (per-tile, not per-group — one duplicate bag can show a date while its sibling shows none). Started as a "bag N of M" counter, changed to roast date since it's actually useful info rather than an arbitrary index — duplicate roast dates are possible but treated as an accepted rare edge case, not solved for.
